...
==========================
TASK_P01: Actualizacion para polydim_runtime_v05.

Cambios respecto a V0.1:
  - Import: polydim_runtime_v03 -> polydim_runtime_v05
  - Agregado: Mesh, META_*, DIM_CONTRACT (10 NATIVE dims)
  - Removido: MockSemanticBackend (no existe en v05)
  - test_INV_003: umbral 0.70->0.75 (DIM_CONTRACT cerca de DIM_META)
  - NUEVO test_HVE_006: B2 numeric encoding proximidad
  - NUEVO test_HVE_007: NATIVE count = 10
  - NUEVO test_binary_roundtrip: roundtrip modo H y modo S
  - NUEVO TestMesh: 6 tests (broadcast, route, topology, hub, route_path, N=5)
  - BUG documentado: Mesh.broadcast/_A/_B fijado al ultimo peer (TASK futura)

Resultados: 38/38 OK (sandbox 2026-06-18)

Uso:
  python polydim_tests_v02.py

Autor:   ai.mpat.agt@gmail.com (curso02.mithril) - TASK_P01 - 2026-06-18
"""
...
class TestObjectNDInvariants(unittest.TestCase):
    ...
    def test_INV_003_subespacios_casi_ortogonales(self):
        """v05: umbral subido a 0.75 por DIM_CONTRACT cerca de DIM_META."""
        sp=Space()
        for i in range(len(NATIVE)):
            for j in range(i+1,len(NATIVE)):
                sim=_sim(sp.sub(NATIVE[i]),sp.sub(NATIVE[j]))
                self.assertLess(sim,0.75,f"INV_003: {NATIVE[i]}<->{NATIVE[j]} sim={sim:.4f}")
    ...
    def test_INV_008_similitud_calculable(self):
        sp=Space("T")
        o1=make_obj(sp,[("DIM_SQL",{},1.0)]); o2=make_obj(sp,[("DIM_SQL",{},1.0),("DIM_PYTHON",{},0.7)])
    ...
    def test_INV_015_superpose_commutative(self):
        ...
        self.assertAlmostEqual(_sim(_sup(a,b),_sup(b,a)),1.0,places=5,msg="INV_015: no conmutativo")
...
class TestHVE(unittest.TestCase):
    """HVE_006/007: B2 numerico y conteo dims nativas. Nuevos en V0.2."""

    def test_HVE_006_b2_numerico_proximidad(self):
        sp=Space()
        h10=sp._enc_numerico(10.0); h11=sp._enc_numerico(11.0); h100=sp._enc_numerico(100.0)
        sim_close=_sim(h10,h11); sim_far=_sim(h10,h100)
        self.assertGreater(sim_close,0.99,f"HVE_006: sim(10,11)={sim_close:.4f} < 0.99")
        self.assertLess(sim_far,sim_close,f"HVE_006: sim(10,100)={sim_far:.4f} >= sim(10,11)")

    def test_HVE_007_native_dims_count(self):
        self.assertEqual(len(NATIVE),10,f"HVE_007: NATIVE tiene {len(NATIVE)}, esperaba 10")
        self.assertIn("DIM_CONTRACT",NATIVE,"HVE_007: DIM_CONTRACT no en NATIVE")
        for d in ["DIM_PYTHON","DIM_RUST","DIM_FLUTTER","DIM_SQL","DIM_GRAPH",
                  "DIM_VECTOR","DIM_TIME","DIM_ERROR","DIM_META"]:
            self.assertIn(d,NATIVE,f"HVE_007: {d} desaparecio de NATIVE")


class TestMesh(unittest.TestCase):
    """MSH_001-006: Mesh (TASK_023/v05). Nuevos en V0.2."""

    def test_MSH_001_broadcast_n3(self):
        """Mismo Space para todos: workaround bug _A/_B multi-peer en broadcast."""
        mesh=Mesh(); sp=Space("SH_BCAST")
        mesh.add("IA_0",sp); mesh.add("IA_1",sp); mesh.add("IA_2",sp)
        mesh.connect_all()
        obj=ObjectND(sp).add("DIM_SQL",{"tabla":"mesh"},w=1.0)
        res=mesh.broadcast(obj,"IA_0")
        self.assertIn("IA_1",res); self.assertIn("IA_2",res)
        self.assertIn("DIM_SQL",res["IA_1"],"MSH_001: IA_1 no recibio DIM_SQL")
        self.assertIn("DIM_SQL",res["IA_2"],"MSH_001: IA_2 no recibio DIM_SQL")

    def test_MSH_002_route_punto_a_punto(self):
        mesh=Mesh(); sp0,sp1,sp2=Space("R0"),Space("R1"),Space("R2")
        mesh.add("R_0",sp0); mesh.add("R_1",sp1); mesh.add("R_2",sp2)
        mesh.connect_all()
        dims=mesh.route(ObjectND(sp0).add("DIM_GRAPH",{"n":"o"},w=1.0),"R_0","R_2")
        self.assertIn("DIM_GRAPH",dims,"MSH_002: DIM_GRAPH no llego a R_2")

    def test_MSH_003_topology_malla_completa_n3(self):
        mesh=Mesh()
        for i in range(3): mesh.add(f"T_{i}",Space(f"T{i}"))
        mesh.connect_all()
        for name,peers in mesh.topology().items():
            self.assertEqual(len(peers),2,f"MSH_003: {name} tiene {len(peers)} peers, esperaba 2")

    def test_MSH_004_hub_topology(self):
        mesh=Mesh(); mesh.add("HUB",Space("HUB"))
        for i in range(3): mesh.add(f"W_{i}",Space(f"W{i}"))
        mesh.connect_hub("HUB"); topo=mesh.topology()
        self.assertEqual(set(topo["HUB"]),{"W_0","W_1","W_2"},"MSH_004: HUB peers incorrectos")
        for i in range(3):
            self.assertEqual(set(topo[f"W_{i}"]),{"HUB"},f"MSH_004: W_{i} tiene peers extras")

    def test_MSH_005_route_path_multisalto(self):
        """Mismo Space: workaround bug _A/_B en route_path multisalto."""
        mesh=Mesh(); sp=Space("SH_PATH")
        mesh.add("A",sp); mesh.add("B",sp); mesh.add("C",sp)
        mesh.connect_all()
        dims=mesh.route_path(ObjectND(sp).add("DIM_TIME",{"o":1},w=1.0),["A","B","C"])
        self.assertIsInstance(dims,dict); self.assertIn("DIM_TIME",dims,"MSH_005: DIM_TIME no llego a C")

    def test_MSH_006_n5_all_ready(self):
        mesh=Mesh()
        for i in range(5): mesh.add(f"M_{i}",Space(f"M{i}"))
        mesh.connect_all()
        for name,info in mesh.status().items():
            self.assertEqual(info["state"],"READY",f"MSH_006: {name} en {info['state']}")
...
class TestIntegracion(unittest.TestCase):
    ...
    def test_transferencia_dims_declaradas(self):
        ...
        obj=make_obj(ia.space,[("DIM_SQL",{"tabla":"u"},1.0),("DIM_PYTHON",{"tipo":"d"},0.7),
                                ("DIM_FLUTTER",{"w":"F"},0.3),("DIM_RUST",{"tipo":"s"},0.0)])
    ...
    def test_dim_contract_transferible(self):
        ...
        self.assertIn("DIM_CONTRACT",obj.dims_activas(),"DIM_CONTRACT no activa en sender")
        self.assertIn("DIM_CONTRACT",conn.transfer(obj),"DIM_CONTRACT no recibida")

    def test_binary_roundtrip(self):
        ia,ib=make_session_pair()
        pkt=ia.send(make_obj(ia.space,[("DIM_SQL",{"t":"x"},1.0),("DIM_PYTHON",{"v":"3"},0.8),("DIM_GRAPH",{"n":"r"},0.6)]))
        self.assertEqual(ia.mode,Mode.H)
        self.assertIsNotNone(pkt.payload_G); self.assertIsNotNone(pkt.payload_S)
        dims=ib.receive(pkt)
        self.assertIsInstance(dims,dict,"receive no retorna dict")
        self.assertIn("DIM_SQL",dims); self.assertIn("DIM_PYTHON",dims)
        ia_s=Session(Space("SA"),"SA",caps=[Cap.S]); ib_s=Session(Space("SB"),"SB",caps=[Cap.S])
        ia_s.connect(ib_s)
        pkt_s=ia_s.send(make_obj(ia_s.space,[("DIM_SQL",{"t":"u"},1.0)]))
        self.assertIsNone(pkt_s.payload_G); self.assertIsNotNone(pkt_s.payload_S)
        self.assertIn("DIM_SQL",ib_s.receive(pkt_s))


if __name__=="__main__":
    suite=unittest.TestSuite()
    for cls in [TestObjectNDInvariants,TestHandshakeInvariants,TestAlignInvariants,
                TestSessionLifecycleInvariants,TestHVE,TestMesh,TestIntegracion]:
